...
struct mcsat_solver_s {
  ...
  /** Root solver if this is a parallel worker replica */
  mcsat_solver_t* parallel_owner;

  /** Parallel run state (owner only) */
  mcsat_parallel_state_t* parallel_state;

  /** Worker identifier for heuristic diversification */
  uint32_t worker_id;

  /** Nesting depth of the shared term lock */
  uint32_t term_lock_depth;
  ...
  /** Private exception boundary for worker replicas */
  jmp_buf worker_env;
  ...
  /** Sequence number just before the current shared lemma session started */
  uint64_t shared_lemma_base_cursor;

  /** Shared lemma session for the current solve */
  uint64_t shared_lemma_session;

  /** Shared fact import cursor */
  uint64_t shared_fact_cursor;

  /** Sequence number just before the current shared fact session started */
  uint64_t shared_fact_base_cursor;

  /** Shared fact session for the current solve */
  uint64_t shared_fact_session;
  ...
  ivector_t pending_model_hint_filter;

  /** Frontier-epoch scheduling mode */
  bool frontier_epoch_mode;
  uint32_t frontier_epoch_conflict_budget;
  uint32_t frontier_epoch_conflicts;
};
...
static
void mcsat_clear_pending_model_hint(mcsat_solver_t* mcsat);

static
void mcsat_reset_shared_sessions(mcsat_solver_t* mcsat,
                                 uint64_t lemma_session,
                                 uint64_t lemma_base_cursor,
                                 uint64_t fact_session,
                                 uint64_t fact_base_cursor);
...
static
void mcsat_import_shared_facts(mcsat_solver_t* mcsat);
...
static inline
mcsat_solver_t* mcsat_parallel_root(mcsat_solver_t* mcsat) {
  return mcsat->parallel_owner == NULL ? mcsat : mcsat->parallel_owner;
}

static inline
bool mcsat_is_parallel_owner(const mcsat_solver_t* mcsat) {
  return mcsat->parallel_owner == NULL;
}

static
bool mcsat_logic_has_arrays(smt_logic_t logic) {
  switch (logic) {
  case AX:
  case ABV:
  case ALIA:
...
static inline
bool mcsat_parallel_problem_is_worth_fanout(const mcsat_solver_t* mcsat) {
  if (mcsat_logic_has_arrays(mcsat->ctx->logic)) {
    return false;
  }
  return variable_db_size(mcsat->var_db) >= MCSAT_PARALLEL_MIN_VARIABLES;
}

static inline
bool mcsat_parallel_is_enabled(const mcsat_solver_t* mcsat) {
  return mcsat_is_parallel_owner(mcsat) &&
         mcsat->ctx->mcsat_options.parallel_workers > 1 &&
         mcsat_parallel_problem_is_worth_fanout(mcsat);
}

static inline
void mcsat_term_lock(mcsat_solver_t* mcsat) {
  mcsat_shared_state_term_lock(mcsat->shared_state);
  mcsat->term_lock_depth ++;
}

static inline
void mcsat_term_unlock(mcsat_solver_t* mcsat) {
  assert(mcsat->term_lock_depth > 0);
  mcsat->term_lock_depth --;
  mcsat_shared_state_term_unlock(mcsat->shared_state);
}

static
void mcsat_term_unlock_all(mcsat_solver_t* mcsat) {
  while (mcsat->term_lock_depth > 0) {
...
static
void mcsat_clear_pending_model_hint(mcsat_solver_t* mcsat) {
  mcsat_solver_t* root = mcsat_parallel_root(mcsat);
  root->pending_model_hint_model = NULL;
  ivector_reset(&root->pending_model_hint_filter);
}
...
void mcsat_reset_shared_sessions(mcsat_solver_t* mcsat,
                                 ...
                                 uint64_t fact_session,
                                 uint64_t fact_base_cursor) {
  mcsat->shared_lemma_session = lemma_session;
  mcsat->shared_lemma_base_cursor = lemma_base_cursor;
  if (lemma_session == 0 || mcsat->shared_state == NULL) {
    mcsat->shared_lemma_cursor = 0;
  } else {
    mcsat->shared_lemma_cursor = lemma_base_cursor;
  }

  mcsat->shared_fact_session = fact_session;
  mcsat->shared_fact_base_cursor = fact_base_cursor;
  if (fact_session == 0 || mcsat->shared_state == NULL) {
    mcsat->shared_fact_cursor = 0;
  } else {
    mcsat->shared_fact_cursor = fact_base_cursor;
  }
}

static
void mcsat_parallel_destroy_winner(mcsat_solver_t* mcsat) {
  mcsat_parallel_state_t* parallel;

  if (!mcsat_is_parallel_owner(mcsat) || mcsat->parallel_state == NULL) {
    return;
  }

  parallel = mcsat->parallel_state;
  if (parallel->winner != NULL && parallel->winner != mcsat) {
    mcsat_destruct(parallel->winner);
    safe_free(parallel->winner);
  }
...
void mcsat_parallel_request_stop_locked(mcsat_parallel_state_t* parallel) {
  ...
  parallel->stop_requested = true;
  if (parallel->workers != NULL) {
    for (i = 0; i < parallel->worker_count; ++i) {
      if (parallel->workers[i] != NULL) {
        parallel->workers[i]->stop_search = true;
      }
    }
  }
...
static
void mcsat_parallel_request_stop(mcsat_parallel_state_t* parallel) {
  if (parallel == NULL) {
    return;
  }
...
void mcsat_parallel_invalidate(mcsat_solver_t* mcsat) {
  ...
  if (root->parallel_state != NULL && root->parallel_state->running) {
    return;
  }

  mcsat_parallel_destroy_winner(root);
  mcsat_reset_shared_sessions(root, 0, 0, 0, 0);
  mcsat_clear_pending_model_hint(root);
}

static
bool mcsat_should_stop(const mcsat_solver_t* mcsat) {
  const mcsat_solver_t* root;
  const mcsat_parallel_state_t* parallel;

  if (mcsat->stop_search) {
    return true;
  }

  root = mcsat->parallel_owner == NULL ? mcsat : mcsat->parallel_owner;
  parallel = root->parallel_state;
  return parallel != NULL && parallel->running && parallel->stop_requested;
}

static
void mcsat_normalize_clause_literals(ivector_t* literals) {
  uint32_t i;
  uint32_t out;
  ...
  int_array_sort(literals->data, literals->size);
  out = 1;
  for (i = 1; i < literals->size; ++i) {
    if (literals->data[i] != literals->data[out - 1]) {
      literals->data[out++] = literals->data[i];
    }
  }
  literals->size = out;
}
...
static
bool mcsat_literal_is_shared_fact_candidate(const mcsat_solver_t* mcsat, term_t literal) {
  term_t atom = unsigned_term(literal);

  return atom != NULL_TERM &&
         atom != true_term &&
         atom != false_term &&
...
static
void mcsat_notify_plugins_of_fact(mcsat_solver_t* mcsat, term_t literal) {
  ivector_t lemma_literals;
  int32_t data[1];

  data[0] = literal;
  lemma_literals.capacity = 1;
  lemma_literals.size = 1;
  lemma_literals.data = data;
  mcsat_notify_plugins_of_lemma(mcsat, &lemma_literals);
}

static
bool mcsat_publish_shared_fact(mcsat_solver_t* mcsat, term_t literal) {
  uint64_t seq = 0;

  if (mcsat->shared_state == NULL ||
      mcsat->shared_fact_session == 0 ||
      !mcsat_literal_is_shared_fact_candidate(mcsat, literal)) {
    return false;
  }

  (void) mcsat_shared_state_publish_fact(mcsat->shared_state,
                                         mcsat->shared_fact_session,
                                         literal,
                                         &seq);
  if (seq > mcsat->shared_fact_cursor) {
    mcsat->shared_fact_cursor = seq;
  }
  return true;
}
...
void mcsat_publish_shared_clause(mcsat_solver_t* mcsat, const ivector_t* lemma) {
  ...
  if (normalized.size > 0 && !mcsat_clause_is_tautology(&normalized)) {
    if (normalized.size == 1 &&
        mcsat_publish_shared_fact(mcsat, normalized.data[0])) {
      delete_ivector(&normalized);
      return;
    }

    if (mcsat->shared_lemma_session != 0) {
      (void) mcsat_shared_state_publish_clause(mcsat->shared_state,
                                               mcsat->shared_lemma_session,
                                               normalized.size,
                                               normalized.data,
                                               &seq);
      if (seq > mcsat->shared_lemma_cursor) {
...
static
bool mcsat_shared_facts_pending(mcsat_solver_t* mcsat) {
  if (mcsat->shared_state == NULL || mcsat->shared_fact_session == 0) {
    return false;
  }
  return mcsat->shared_fact_cursor < mcsat_shared_state_latest_fact_seq(mcsat->shared_state);
}

static
bool mcsat_shared_lemmas_pending(mcsat_solver_t* mcsat) {
  if (mcsat->shared_state == NULL || mcsat->shared_lemma_session == 0) {
    return false;
  }
...
static
void mcsat_install_shared_fact(mcsat_solver_t* mcsat, term_t literal) {
  term_t atom;
  ...
  (void) variable_db_get_variable(mcsat->var_db, atom);
  mcsat_process_registration_queue(mcsat);
  mcsat_notify_plugins_of_fact(mcsat, literal);
}

static
void mcsat_install_shared_clause(mcsat_solver_t* mcsat, uint32_t n_literals, const term_t literals[]) {
  ivector_t lemma_literals;
  uint32_t i;

  init_ivector(&lemma_literals, 0);
  ivector_add(&lemma_literals, literals, n_literals);
  if (lemma_literals.size == 0) {
    delete_ivector(&lemma_literals);
    return;
  }

  for (i = 0; i < lemma_literals.size; ++i) {
    term_t atom = unsigned_term(lemma_literals.data[i]);
    if (atom != NULL_TERM) {
      (void) variable_db_get_variable(mcsat->var_db, atom);
    }
  }
...
void mcsat_import_shared_facts(mcsat_solver_t* mcsat) {
  ...
  target = mcsat_shared_state_latest_fact_seq(mcsat->shared_state);
  for (seq = mcsat->shared_fact_cursor + 1; seq <= target && mcsat_is_consistent(mcsat); ++seq) {
    term_t literal = mcsat_shared_state_get_fact(mcsat->shared_state,
                                                 mcsat->shared_fact_session,
                                                 seq);
    if (literal == NULL_TERM) {
      continue;
    }
    mcsat_install_shared_fact(mcsat, literal);
  }

  mcsat->shared_fact_cursor = target;
}
...
void mcsat_import_shared_lemmas(mcsat_solver_t* mcsat) {
  ...
  for (seq = mcsat->shared_lemma_cursor + 1; seq <= target && mcsat_is_consistent(mcsat); ++seq) {
    const term_t* literals = NULL;
    uint32_t n_literals = mcsat_shared_state_get_clause(mcsat->shared_state,
                                                        mcsat->shared_lemma_session,
                                                        seq,
                                                        &literals);
    if (n_literals == 0 || literals == NULL) {
      continue;
    }
    mcsat_install_shared_clause(mcsat, n_literals, literals);
  }
...
void mcsat_heuristics_init(mcsat_solver_t* mcsat, const param_t *params) {
  static const uint32_t restart_intervals[] = { 10, 18, 32, 56 };
  static const uint32_t recache_intervals[] = { 300, 220, 420, 640 };
  static const lemma_weight_type_t lemma_weights[] = {
    LEMMA_WEIGHT_SIZE,
    LEMMA_WEIGHT_GLUE,
    LEMMA_WEIGHT_UNIT,
    LEMMA_WEIGHT_GLUE
  };
  static const double portfolio_randomness[] = { 0.0, 0.08, 0.18, 0.35 };
  uint32_t portfolio_i = mcsat->worker_id % 4;
  double random_freq = params->randomness;

  mcsat->heuristic_params.restart_interval = restart_intervals[portfolio_i];
  mcsat->heuristic_params.lemma_restart_weight_type = lemma_weights[portfolio_i];
  mcsat->heuristic_params.recache_interval = recache_intervals[portfolio_i];
  if (random_freq < portfolio_randomness[portfolio_i]) {
    random_freq = portfolio_randomness[portfolio_i];
  }
  mcsat->heuristic_params.random_decision_freq = random_freq;
  mcsat->heuristic_params.random_decision_seed = params->random_seed + (7919u * mcsat->worker_id);
}
  ...
  if (!is_equality) {
    for (i = kind; mcsat->kind_owners[i] != MCSAT_MAX_PLUGINS; i += NUM_TERM_KINDS) {
      ...
      if (plugin->explain_evaluation) {
        ...
        mcsat_term_lock((mcsat_solver_t*) mcsat);
        evaluates = plugin->explain_evaluation(plugin, t, vars, value);
        mcsat_term_unlock((mcsat_solver_t*) mcsat);
        if (evaluates) {
          return true;
        }
      }
...
static
void mcsat_parallel_state_construct(mcsat_parallel_state_t* parallel) {
  int32_t code;

  code = pthread_mutex_init(&parallel->result_lock, NULL);
  if (code != 0) {
...
static
void mcsat_construct(mcsat_solver_t* mcsat, const context_t* ctx, mcsat_solver_t* owner, uint32_t worker_id) {
  uint32_t i;
  ...
  mcsat->parallel_owner = owner;
  mcsat->parallel_state = NULL;
  mcsat->worker_id = worker_id;
  mcsat->term_lock_depth = 0;
  mcsat->stop_search = false;
  mcsat->ctx = ctx;
  mcsat->exception = owner == NULL ? (jmp_buf*) &ctx->env : &mcsat->worker_env;
  ...
  mcsat->shared_lemma_cursor = 0;
  mcsat->shared_lemma_base_cursor = 0;
  mcsat->shared_lemma_session = 0;
  mcsat->shared_fact_cursor = 0;
  mcsat->shared_fact_base_cursor = 0;
  mcsat->shared_fact_session = 0;
  mcsat->pending_model_hint_model = NULL;
  init_ivector(&mcsat->pending_model_hint_filter, 0);
  ...
  if (owner == NULL) {
    mcsat->parallel_state = safe_malloc(sizeof(mcsat_parallel_state_t));
    mcsat_parallel_state_construct(mcsat->parallel_state);
  }
}

static
void mcsat_destruct_local(mcsat_solver_t* mcsat) {
  uint32_t i;
  plugin_t* plugin;
  ...
  delete_ivector(&mcsat->pending_model_hint_filter);
...
mcsat_solver_t* mcsat_new(const context_t* ctx) {
  ...
  mcsat_construct(mcsat, ctx, NULL, 0);
  return mcsat;
}

void mcsat_destruct(mcsat_solver_t* mcsat) {
  if (mcsat_is_parallel_owner(mcsat)) {
    mcsat_parallel_destroy_winner(mcsat);
    if (mcsat->parallel_state != NULL) {
      mcsat_parallel_state_destruct(mcsat->parallel_state);
      safe_free(mcsat->parallel_state);
      mcsat->parallel_state = NULL;
    }
  }
  mcsat_destruct_local(mcsat);
}
...
static
void mcsat_reset_local(mcsat_solver_t* mcsat) {
  // Reset everything
  const context_t* ctx = mcsat->ctx;
  mcsat_solver_t* owner = mcsat->parallel_owner;
  uint32_t worker_id = mcsat->worker_id;
  mcsat_parallel_state_t* parallel_state = mcsat->parallel_state;
  if (owner == NULL && parallel_state != NULL) {
    mcsat_parallel_state_destruct(parallel_state);
    safe_free(parallel_state);
    mcsat->parallel_state = NULL;
  }
  mcsat_destruct_local(mcsat);
  mcsat_construct(mcsat, ctx, owner, worker_id);
}

void mcsat_reset(mcsat_solver_t* mcsat) {
  if (mcsat_is_parallel_owner(mcsat)) {
    mcsat_parallel_invalidate(mcsat);
  }
  mcsat_reset_local(mcsat);
}
...
static
void mcsat_push_local(mcsat_solver_t* mcsat) {
...
void mcsat_push(mcsat_solver_t* mcsat) {
  ...
  mcsat_push_local(mcsat);
}

static
void mcsat_pop_local(mcsat_solver_t* mcsat) {
  ...
  if (definition_lemmas_size < mcsat->plugin_definition_lemmas.size) {
    mcsat_assert_formulas_internal(mcsat, 0, NULL, true);
  }
...
void mcsat_pop(mcsat_solver_t* mcsat) {
  ...
  mcsat_pop_local(mcsat);
}

static
void mcsat_clear_local(mcsat_solver_t* mcsat) {
...
static
bool mcsat_term_is_prebuilt_input_term(const mcsat_solver_t* mcsat, term_t term) {
  term = unsigned_term(term);
  return term != NULL_TERM && term < mcsat->terms_size_on_solver_entry;
}

static
bool mcsat_term_is_safe_split_term(const mcsat_solver_t* mcsat, term_t term) {
  term_kind_t kind;
  ...
  if (term == NULL_TERM ||
      term == true_term ||
      term == false_term ||
      !mcsat_term_is_prebuilt_input_term(mcsat, term) ||
      term_type_kind(mcsat->terms, term) != BOOL_TYPE) {
    return false;
  }

  kind = term_kind(mcsat->terms, term);
  switch (kind) {
  case VARIABLE:
...
bool mcsat_parallel_try_add_boolean_split_variable(mcsat_solver_t* mcsat, variable_t var, ivector_t* split_atoms, int_hset_t* seen_terms) {
  ...
  term = variable_db_get_term(mcsat->var_db, var);
  if (!mcsat_term_is_safe_split_term(mcsat, term)) {
    return false;
  }
...
void mcsat_parallel_prepare_term_state(mcsat_solver_t* mcsat) {
  ...
  for (i = 1; i < mcsat->var_db->variable_to_term_map.size; ++i) {
    term_t term = mcsat->var_db->variable_to_term_map.data[i];
    if (mcsat_term_is_prebuilt_input_term(mcsat, term)) {
      mcsat_shared_state_publish_term(mcsat->shared_state, term);
    }
  }

  for (i = 0; i < mcsat->assertion_terms_original.size; ++i) {
    term_t term = unsigned_term(mcsat->assertion_terms_original.data[i]);
    if (mcsat_term_is_prebuilt_input_term(mcsat, term)) {
      mcsat_shared_state_publish_term(mcsat->shared_state, term);
    }
  }
...
bool mcsat_propagate(mcsat_solver_t* mcsat, bool run_learning) {
  ...
  do {
    if (mcsat_shared_facts_pending(mcsat)) {
      mcsat_import_shared_facts(mcsat);
    }
    if (mcsat_shared_lemmas_pending(mcsat)) {
      mcsat_import_shared_lemmas(mcsat);
    }
...
void mcsat_add_lemma(mcsat_solver_t* mcsat, ivector_t* lemma, term_t decision_bound) {
  ...
  mcsat_publish_shared_clause(mcsat, lemma);
  ...
  mcsat_notify_plugins_of_lemma(mcsat, lemma);
...
term_t mcsat_analyze_final(mcsat_solver_t* mcsat, conflict_t* input_conflict) {
  ...
  *mcsat->trail = saved_trail;
  ...
  term_t interpolant;
  if (literals.size == 0) {
    interpolant = true_term;
  } else {
    mcsat_term_lock(mcsat);
    interpolant = mk_and(&mcsat->tm, literals.size, literals.data);
    mcsat_term_unlock(mcsat);
  }
...
bool mcsat_apply_literal_assumptions(mcsat_solver_t* mcsat, uint32_t n_assumptions, const term_t assumptions[]) {
  ...
  for (i = 0; i < n_assumptions; ++i) {
    ...
    assert(term_type_kind(mcsat->terms, atom) == BOOL_TYPE);
    value = (literal == atom) ? &mcsat_value_true : &mcsat_value_false;

    var = variable_db_get_variable(mcsat->var_db, atom);
    ivector_push(&mcsat->assumption_vars, var);
    mcsat_process_registration_queue(mcsat);

    if (trail_has_value(mcsat->trail, var)) {
      assert(trail_get_value(mcsat->trail, var)->type == VALUE_BOOLEAN);
      if (trail_get_boolean_value(mcsat->trail, var) != value->b) {
        uint32_t plugin_i = trail_get_source_id(mcsat->trail, var);
        if (plugin_i == MCSAT_MAX_PLUGINS) {
          mcsat->plugin_in_conflict = NULL;
        } else {
          mcsat->plugin_in_conflict = mcsat->plugins[plugin_i].plugin_ctx;
    ...
    assert(plugin->decide_assignment);
    mcsat_term_lock(mcsat);
    plugin->decide_assignment(plugin, var, value, (trail_token_t*) &decision_token);
    mcsat_term_unlock(mcsat);
...
static
void mcsat_set_model_hint_local(mcsat_solver_t* mcsat, model_t* mdl, uint32_t n_mdl_filter,
...
static
void mcsat_solve_local(mcsat_solver_t* mcsat, const param_t *params, model_t* mdl, uint32_t n_assumptions, const term_t assumptions[]) {
  ...
  if (n_assumptions > 0) {
    ...
    if (mdl == NULL) {
      if (!mcsat_apply_literal_assumptions(mcsat, n_assumptions, assumptions)) {
        goto assumptions_done;
      }
    } else {
      uint32_t i;
      for (i = 0; i < n_assumptions; ++ i) {
...
assumptions_done:
  ...
  while (!mcsat_should_stop(mcsat)) {
    ...
    bool assumption_decided = false;
    if (!(mdl == NULL && n_assumptions > 0)) {
      assumption_decided = mcsat_decide_assumption(mcsat, mdl, n_assumptions, assumptions);
    }
  ...
  conflict:
    ...
    if (mcsat->frontier_epoch_mode &&
        mcsat->status == YICES_STATUS_SEARCHING &&
        mcsat->frontier_epoch_conflict_budget > 0) {
      mcsat->frontier_epoch_conflicts ++;
      if (mcsat->frontier_epoch_conflicts >= mcsat->frontier_epoch_conflict_budget) {
        mcsat->status = YICES_STATUS_IDLE;
        break;
      }
    }
...
static
uint32_t mcsat_scope_depth(const mcsat_solver_t* mcsat) {
  if (mcsat->scope.push_size == 0) {
    return 0;
  }
  return mcsat->scope.trace.size / mcsat->scope.push_size;
}

static
uint32_t mcsat_scope_assertion_boundary(const mcsat_solver_t* mcsat, uint32_t level) {
  assert(mcsat->scope.push_size >= 2);
  return (uint32_t) mcsat->scope.trace.data[level * mcsat->scope.push_size + 1];
}

static
void mcsat_replay_owner_state(mcsat_solver_t* dst, const mcsat_solver_t* src) {
  uint32_t level_count;
  uint32_t prev_assertions;
  uint32_t level;

  assert(dst->status == YICES_STATUS_IDLE);
  assert(src->status == YICES_STATUS_IDLE);

  /*
   * Rebuild a worker from the owner-visible incremental state so replicas keep
   * their own mutable trail/plugin state while sharing the global registries.
   */
  level_count = mcsat_scope_depth(src);
  prev_assertions = 0;
  for (level = 0; level < level_count; ++level) {
    uint32_t boundary = mcsat_scope_assertion_boundary(src, level);
    if (boundary > prev_assertions) {
      mcsat_assert_formulas_internal(dst,
                                     boundary - prev_assertions,
                                     src->assertion_terms_original.data + prev_assertions,
                                     true);
    }
    mcsat_push_local(dst);
    prev_assertions = boundary;
  }

  if (src->assertion_terms_original.size > prev_assertions) {
    mcsat_assert_formulas_internal(dst,
                                   src->assertion_terms_original.size - prev_assertions,
                                   src->assertion_terms_original.data + prev_assertions,
                                   true);
  }
}
...
static
mcsat_solver_t* mcsat_new_worker_replica(mcsat_solver_t* owner, uint32_t worker_id) {
  mcsat_solver_t* worker;
  int32_t code;

  worker = (mcsat_solver_t*) safe_malloc(sizeof(mcsat_solver_t));
  mcsat_construct(worker, owner->ctx, owner, worker_id);
  code = setjmp(worker->worker_env);
  if (code == 0) {
    mcsat_replay_owner_state(worker, owner);
  } else {
    mcsat_term_unlock_all(worker);
    mcsat_destruct_local(worker);
    safe_free(worker);
    fprintf(stderr, "parallel MCSAT worker replay failed\n");
    abort();
  }
  return worker;
}
...
static
void mcsat_parallel_destroy_replicas(mcsat_solver_t** workers, uint32_t count, mcsat_solver_t* keep) {
  uint32_t i;
...
void mcsat_solve_parallel(mcsat_solver_t* mcsat, const param_t *params, model_t* mdl, uint32_t n_assumptions, const term_t assumptions[]) {
  ...
  requested_workers = (uint32_t) mcsat->ctx->mcsat_options.parallel_workers;
  lemma_base_cursor = mcsat->shared_state == NULL ? 0 : mcsat_shared_state_latest_lemma_seq(mcsat->shared_state);
  lemma_session = mcsat->shared_state == NULL ? 0 : mcsat_shared_state_new_lemma_session(mcsat->shared_state);
  fact_base_cursor = mcsat->shared_state == NULL ? 0 : mcsat_shared_state_latest_fact_seq(mcsat->shared_state);
  fact_session = mcsat->shared_state == NULL ? 0 : mcsat_shared_state_new_fact_session(mcsat->shared_state);
  if (requested_workers <= 1 ||
      n_assumptions > 0 ||
      mcsat->ctx->mcsat_options.model_interpolation) {
    mcsat_reset_shared_sessions(mcsat,
                                lemma_session,
                                lemma_base_cursor,
                                fact_session,
                                fact_base_cursor);
  ...
  for (i = 0; i < active_count; ++i) {
    ...
    mcsat_reset_shared_sessions(workers[i],
                                lemma_session,
                                lemma_base_cursor,
                                fact_session,
                                fact_base_cursor);
...
void mcsat_set_model_hint(mcsat_solver_t* mcsat, model_t* mdl, uint32_t n_mdl_filter,
  ...
  if (mcsat_is_parallel_owner(mcsat)) {
    mcsat_parallel_destroy_winner(mcsat);
    mcsat->pending_model_hint_model = mdl;
    ivector_copy(&mcsat->pending_model_hint_filter, mdl_filter, n_mdl_filter);
  }
  mcsat_set_model_hint_local(mcsat, mdl, n_mdl_filter, mdl_filter);
}
...
void mcsat_solve(mcsat_solver_t* mcsat, const param_t *params, model_t* mdl, uint32_t n_assumptions, const term_t assumptions[]) {
  ...
  if (mcsat_parallel_is_enabled(mcsat)) {
    mcsat_solve_parallel(mcsat, params, mdl, n_assumptions, assumptions);
  } else {
    uint64_t lemma_base_cursor = mcsat_shared_state_latest_lemma_seq(mcsat->shared_state);
    uint64_t lemma_session = mcsat_shared_state_new_lemma_session(mcsat->shared_state);
    uint64_t fact_base_cursor = mcsat_shared_state_latest_fact_seq(mcsat->shared_state);
    uint64_t fact_session = mcsat_shared_state_new_fact_session(mcsat->shared_state);
    mcsat_reset_shared_sessions(mcsat,
                                lemma_session,
                                lemma_base_cursor,
                                fact_session,
...
void mcsat_set_tracer(mcsat_solver_t* mcsat, tracer_t* tracer) {
  ...
  if (mcsat_is_parallel_owner(mcsat) &&
      mcsat->parallel_state != NULL &&
      mcsat->parallel_state->winner != NULL &&
      mcsat->parallel_state->winner != mcsat) {
    mcsat_set_tracer(mcsat->parallel_state->winner, tracer);
  }
}
...
void mcsat_show_stats(mcsat_solver_t* mcsat, FILE* out) {
  if (mcsat_is_parallel_owner(mcsat) &&
      ...
      mcsat->parallel_state->winner != NULL &&
      mcsat->parallel_state->winner != mcsat) {
    mcsat = mcsat->parallel_state->winner;
  }
  int fd = fileno(out);
  assert(fd >= 0);
...
void mcsat_set_exception_handler(mcsat_solver_t* mcsat, jmp_buf* handler) {
  ...
  if (mcsat_is_parallel_owner(mcsat) &&
      ...
      mcsat->parallel_state->winner != NULL &&
      mcsat->parallel_state->winner != mcsat) {
    mcsat_set_exception_handler(mcsat->parallel_state->winner, handler);
  }
}
...
void mcsat_stop_search(mcsat_solver_t* mcsat) {
  ...
  if (mcsat_is_parallel_owner(mcsat) && mcsat->parallel_state != NULL) {
    mcsat_parallel_request_stop(mcsat->parallel_state);
  }
}
