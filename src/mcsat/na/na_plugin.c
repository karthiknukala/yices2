...
static
void na_plugin_stats_init(na_plugin_t* na) {
  // Add statistics
  na->stats.propagations = statistics_new_int(na->ctx->stats, "mcsat::na::propagations");
  na->stats.conflicts = statistics_new_int(na->ctx->stats, "mcsat::na::conflicts");
  na->stats.conflicts_int = statistics_new_int(na->ctx->stats, "mcsat::na::conflicts_int");
  na->stats.conflicts_assumption = statistics_new_int(na->ctx->stats, "mcsat::na::conflicts_assumption");
  na->stats.constraints_attached = statistics_new_int(na->ctx->stats, "mcsat::na::constraints_attached");
...
static
void na_plugin_heuristics_init(na_plugin_t* na) {
  // Initialize heuristic
}

static
void na_plugin_construct(plugin_t* plugin, plugin_context_t* ctx) {
  ...
  init_ivector(&na->processed_variables, 0);
  na->processed_variables_size = 0;

  scope_holder_construct(&na->scope);

  init_int_hmap(&na->evaluation_value_cache, 0);
  init_int_hmap(&na->evaluation_timestamp_cache, 0);

  init_int_hmap(&na->feasible_set_cache_top_var[0], 0);
  init_int_hmap(&na->feasible_set_cache_top_var[1], 0);
  init_int_hmap(&na->feasible_set_cache_timestamp[0], 0);
  init_int_hmap(&na->feasible_set_cache_timestamp[1], 0);
  init_ptr_hmap(&na->feasible_set_cache[0], 0);
  init_ptr_hmap(&na->feasible_set_cache[1], 0);

  // Constraint db
  na->constraint_db = poly_constraint_db_new(&na->lp_data);

  // Feasible sets
  na->feasible_set_db = feasible_set_db_new(na);
  ...
  init_rba_buffer(&na->buffer, ctx->terms->pprods);

  na->global_bound_term = NULL_TERM;

  na_plugin_stats_init(na);
  na_plugin_heuristics_init(na);
}

static
void na_plugin_destruct(plugin_t* plugin) {
  na_plugin_t* na = (na_plugin_t*) plugin;

  watch_list_manager_destruct(&na->wlm);
  ...
  delete_int_hmap(&na->evaluation_timestamp_cache);

  delete_int_hmap(&na->feasible_set_cache_top_var[0]);
  delete_int_hmap(&na->feasible_set_cache_top_var[1]);
  delete_int_hmap(&na->feasible_set_cache_timestamp[0]);
  delete_int_hmap(&na->feasible_set_cache_timestamp[1]);

  // delete feasibility objects
  ptr_hmap_pair_t* it = ptr_hmap_first_record(&na->feasible_set_cache[0]);
  for (; it != NULL; it = ptr_hmap_next_record(&na->feasible_set_cache[0], it)) {
    lp_feasibility_set_delete(it->val);
  }
  delete_ptr_hmap(&na->feasible_set_cache[0]);
  it = ptr_hmap_first_record(&na->feasible_set_cache[1]);
  for (; it != NULL; it = ptr_hmap_next_record(&na->feasible_set_cache[1], it)) {
    lp_feasibility_set_delete(it->val);
  }
  delete_ptr_hmap(&na->feasible_set_cache[1]);
...
lp_feasibility_set_t* na_plugin_get_feasible_set(na_plugin_t* na, variable_t cstr_var, variable_t cstr_top_var, bool is_negated) {
  ...
  const poly_constraint_t* cstr = poly_constraint_db_get(na->constraint_db, cstr_var);

  // Constraint var list (we only cache if in use)
  if (!watch_list_manager_has_constraint(&na->wlm, cstr_var)) {
    return poly_constraint_get_feasible_set(cstr, na->lp_data.lp_assignment, is_negated);
  }
  variable_list_ref_t var_list_ref = watch_list_manager_get_list_of(&na->wlm, cstr_var);
  const variable_t* var_list = watch_list_manager_get_list(&na->wlm, var_list_ref);
  ...
  while (*var_i != variable_null) {
    if (na_plugin_has_assignment(na, *var_i)) {
  ...
  // Cached top variable
  int_hmap_t* cache_top_var = &na->feasible_set_cache_top_var[is_negated];
  int_hmap_pair_t* find_top_var = int_hmap_get(cache_top_var, cstr_var);
  // Cached timestamp
  int_hmap_t* cache_timestamp = &na->feasible_set_cache_timestamp[is_negated];
  int_hmap_pair_t* find_timestamp = int_hmap_get(cache_timestamp, cstr_var);
  // Cached feasible set
  ptr_hmap_t* cache = &na->feasible_set_cache[is_negated];
  ...
  // Compute
  lp_feasibility_set_t* feasible = poly_constraint_get_feasible_set(cstr, na->lp_data.lp_assignment, is_negated);
...
static
const mcsat_value_t* na_plugin_constraint_evaluate(na_plugin_t* na, variable_t cstr_var, uint32_t* cstr_level) {
  ...
  // Get the timestamp and level
  uint32_t cstr_timestamp = 0;
  *cstr_level = na->ctx->trail->decision_level_base;
  const mcsat_trail_t* trail = na->ctx->trail;
  const variable_t* var_i = var_list;
  while (*var_i != variable_null) {
    if (na_plugin_has_assignment(na, *var_i)) {
  ...
  // Check the cache
  int_hmap_pair_t* find_value = int_hmap_find(&na->evaluation_value_cache, cstr_var);
  int_hmap_pair_t* find_timestamp = NULL;
  if (find_value != NULL) {
    find_timestamp = int_hmap_find(&na->evaluation_timestamp_cache, cstr_var);
  ...
  bool ok = poly_constraint_evaluate(cstr, &na->lp_data, &cstr_value);
  (void) ok;
  assert(ok);
  (*na->stats.evaluations) ++;
  ...
  if (find_value != NULL) {
    ...
    int_hmap_add(&na->evaluation_value_cache, cstr_var, cstr_value);
    int_hmap_add(&na->evaluation_timestamp_cache, cstr_var, cstr_timestamp);
  }
...
static
void na_plugin_process_fully_assigned_constraint(na_plugin_t* na, trail_token_t* prop, variable_t cstr_var) {
  ...
  if (ctx_trace_enabled(na->ctx, "na::evaluate")) {
    trail_print(na->ctx->trail, ctx_trace_out(na->ctx));
    ctx_trace_term(na->ctx, variable_db_get_term(na->ctx->var_db, cstr_var));
  }

  // Compute the evaluation timestamp
  cstr_value = na_plugin_constraint_evaluate(na, cstr_var, &cstr_level);
...
void na_plugin_new_term_notify(plugin_t* plugin, term_t t, trail_token_t* prop) {
  ...
  if (ctx_trace_enabled(na->ctx, "mcsat::new_term")) {
    ctx_trace_printf(na->ctx, "na_plugin_new_term_notify: ");
  ...
  // The variable
  variable_t t_var = variable_db_get_variable(na->ctx->var_db, t);
  ...
  case ARITH_MOD: {
    ...
    variable_db_get_variable(na->ctx->var_db, t_div);
    break;
  }
  case ARITH_IDIV: {
    ...
    variable_db_get_variable(na->ctx->var_db, r);
  ...
  if (is_constraint) {
    ...
    for (i = 0; i < t_variables_list->size; ++ i) {
      ...
      if (!lp_data_variable_has_term(&na->lp_data, tt)) {
        lp_data_add_lp_variable(&na->lp_data, terms, tt);
      }
    }
    ...
    int_array_sort2(t_variables_list->data, t_variables_list->size, (void*) na->ctx->trail, na_plugin_trail_variable_compare);

    if (ctx_trace_enabled(na->ctx, "mcsat::new_term")) {
      ctx_trace_printf(na->ctx, "na_plugin_new_term_notify: vars: \n");
      for (i = 0; i < t_variables_list->size; ++ i) {
        ctx_trace_term(na->ctx, variable_db_get_term(na->ctx->var_db, t_variables_list->data[i]));
      }
    }

    // Make the variable list
    variable_list_ref_t var_list = watch_list_manager_new_list(&na->wlm, t_variables_list->data, t_variables_list->size, t_var);
    ...
    constraint_unit_info_set(&na->unit_info, t_var, unit_status == CONSTRAINT_UNIT ? top_var : variable_null, unit_status);
    ...
    case ARITH_CONSTANT:  {
      ...
      prop->add_at_level(prop, t_var, &mcsat_value, na->ctx->trail->decision_level_base);
    ...
    default: {
      ...
      variable_db_get_variable(na->ctx->var_db, t);
      // register lp_variable for t if not existent
      if (!lp_data_variable_has_term(&na->lp_data, t)) {
        lp_data_add_lp_variable(&na->lp_data, terms, t);
      }

      if (na->ctx->options->na_bound) {
        if (na->global_bound_term == NULL_TERM) {
          type_t reals = int_type(terms->types);
          na->global_bound_term = new_uninterpreted_term(terms, reals);
          set_term_name(terms, na->global_bound_term, clone_string("__mcsat_B"));
        }

        // Add bound lemma b - t >= 0 && t + b >= 0
        term_t ub_term = _o_yices_sub(na->global_bound_term, t);
        term_t lb_term = _o_yices_add(na->global_bound_term, t);
        ...
        if (t == na->global_bound_term) {
          na->ctx->request_top_decision(na->ctx, t_var);
          if (na->ctx->options->na_bound_min >= 0) {
            rational_t q;
            q_init(&q);
            q_set32(&q, na->ctx->options->na_bound_min);
            term_t min = mk_arith_constant(na->ctx->tm, &q);
            term_t min_bound = _o_yices_arith_geq_atom(na->global_bound_term, min);
            prop->lemma(prop, min_bound);
            q_clear(&q);
          }
          if (na->ctx->options->na_bound_max >= 0) {
            rational_t q;
            q_init(&q);
            q_set32(&q, na->ctx->options->na_bound_max);
            term_t max = mk_arith_constant(na->ctx->tm, &q);
            term_t max_bound = _o_yices_arith_leq_atom(na->global_bound_term, max);
...
static
void na_plugin_infer_bounds_from_constraint(na_plugin_t* na, trail_token_t* prop, variable_t constraint_var) {

  uint32_t i;

  // Just at base level for now
  if (!trail_is_at_base_level(na->ctx->trail)) {
    return;
  }

  if (ctx_trace_enabled(na->ctx, "mcsat::na::learn")) {
    ctx_trace_printf(na->ctx, "na infer bounds: constraint :\n");
  ...
  bool trail_value = trail_get_boolean_value(na->ctx->trail, constraint_var);
  ...
  if (conflict) {
    if (ctx_trace_enabled(na->ctx, "mcsat::na::learn")) {
      ctx_trace_printf(na->ctx, "na infer bounds: conflict\n");
    }
    na_plugin_report_conflict(na, prop, constraint_var);
  } else {
    if (ctx_trace_enabled(na->ctx, "mcsat::na::learn")) {
      ctx_trace_printf(na->ctx, "na infer bounds: learned :\n");
      lp_interval_assignment_print(m, ctx_trace_out(na->ctx));
    ...
    for (i = 0; i < lp_variables.size; ++ i) {
      ...
      if (!lp_interval_is_full(x_interval)) {
        term_t x_term = lp_data_get_term_from_lp_variable(&na->lp_data, x_lp);
        variable_t x = variable_db_get_variable(na->ctx->var_db, x_term);
        assert(x != variable_null);
        lp_feasibility_set_t* x_feasible = lp_feasibility_set_new_from_interval(x_interval);
        bool consistent = feasible_set_db_update(na->feasible_set_db, x, x_feasible, &constraint_var, 1);
        if (!consistent) {
          na_plugin_report_conflict(na, prop, constraint_var);
        } else if (variable_db_is_int(na->ctx->var_db, x) && !lp_feasibility_set_contains_int(feasible_set_db_get(na->feasible_set_db, x))) {
          // If x is an integer, we must check that there are integers in the interval.
          na_plugin_report_int_conflict(na, prop, x);
        }
      }
    }
 ...
 */
static
void na_plugin_process_unit_constraint(na_plugin_t* na, trail_token_t* prop, variable_t constraint_var) {

  if (ctx_trace_enabled(na->ctx, "na::propagate")) {
    ctx_trace_printf(na->ctx, "na: processing unit constraint :\n");
    ctx_trace_term(na->ctx, variable_db_get_term(na->ctx->var_db, constraint_var));
  }

  // Process if constraint is assigned, or an evaluation constraint
  bool is_eval_constraint = !variable_db_is_boolean(na->ctx->var_db, constraint_var);
  if (is_eval_constraint || trail_has_value(na->ctx->trail, constraint_var)) {
    // Get the constraint value
    bool constraint_value = is_eval_constraint || trail_get_value(na->ctx->trail, constraint_var)->b;
    ...
    variable_t x = constraint_unit_info_get_unit_var(&na->unit_info, constraint_var);
    assert(x != variable_null);
    ...
    bool still_feasible = feasible_set_db_update(na->feasible_set_db, x, constraint_feasible, &constraint_var, 1);

    if (ctx_trace_enabled(na->ctx, "na::propagate")) {
      ctx_trace_printf(na->ctx, "na: new feasible = ");
      lp_feasibility_set_print(feasible_set_db_get(na->feasible_set_db, x), ctx_trace_out(na->ctx));
      ctx_trace_printf(na->ctx, "\n");
    }

    // If the intervals are empty, we have a conflict
    if (!still_feasible) {
      na_plugin_report_conflict(na, prop, x);
      return;
    }

    const lp_feasibility_set_t *feasible_set = feasible_set_db_get(na->feasible_set_db, x);
    bool x_is_int_var = variable_db_is_int(na->ctx->var_db, x);
    ...
    if (lp_value_is_rational(&x_value)) {
      if (lp_feasibility_set_is_point(feasible_set) || (x_is_int_var && lp_feasibility_set_is_point_int(feasible_set))) {
        ...
        if (trail_is_at_base_level(na->ctx->trail) && !na->ctx->options->model_interpolation) {
          prop->add_at_level(prop, x, &value, na->ctx->trail->decision_level_base);
        } else {
          if (ctx_trace_enabled(na->ctx, "na::propagate")) {
            ctx_trace_printf(na->ctx, "na: hinting variable = %d\n", x);
          }
          na->ctx->hint_next_decision(na->ctx, x);
          if (trail_is_at_base_level(na->ctx->trail)) {
            na->ctx->hint_value(na->ctx, x, &value);
          }
        }
 ...
 */
static
void na_plugin_process_variable_assignment(na_plugin_t* na, trail_token_t* prop, variable_t var) {
  ...
  ivector_push(&na->processed_variables, var);
  na->processed_variables_size ++;

  // Check if we have processed our last decision
  if (var == na->last_decided_and_unprocessed) {
    na->last_decided_and_unprocessed = variable_null;
  }

  term_t t = variable_db_get_term(na->ctx->var_db, var);
  if (ctx_trace_enabled(na->ctx, "na::propagate")) {
    ctx_trace_printf(na->ctx, "na: processing var assignment of :\n");
    ctx_trace_term(na->ctx, t);
  }
  ...
  lp_data_add_to_model_and_context(&na->lp_data, lp_data_get_lp_variable_from_term(&na->lp_data, t), &trail_get_value(trail, var)->lp_value);

  if (ctx_trace_enabled(na->ctx, "na::propagate")) {
    ctx_trace_printf(na->ctx, "na: var order :");
    lp_data_variable_order_print(&na->lp_data, ctx_trace_out(na->ctx));
    ctx_trace_printf(na->ctx, "\n");
  }

  if (ctx_trace_enabled(na->ctx, "na::wlm")) {
    watch_list_manager_print(&na->wlm, ctx_trace_out(na->ctx));
  }

  // Get the watch-list
  remove_iterator_construct(&it, &na->wlm, var);
  ...
  while (!remove_iterator_done(&it)) {
    ...
    var_list = watch_list_manager_get_list(&na->wlm, var_list_ref);

    // Constraint variable
    variable_t constraint_var = watch_list_manager_get_constraint(&na->wlm, var_list_ref);

    if (ctx_trace_enabled(na->ctx, "na::propagate")) {
      ctx_trace_printf(na->ctx, "na: processing constraint :");
      ctx_trace_term(na->ctx, variable_db_get_term(na->ctx->var_db, constraint_var));

      ctx_trace_printf(na->ctx, "na: var_list :");
      var_list_it = var_list;
      while (*var_list_it != variable_null) {
        ctx_trace_term(na->ctx, variable_db_get_term(na->ctx->var_db, *var_list_it));
        var_list_it ++;
      }
    }
    ...
    if (*var_list_it != variable_null) {
      for (++ var_list_it; *var_list_it != variable_null; ++ var_list_it) {
        if (!na_plugin_has_assignment(na, *var_list_it)) {
          // Swap with var_list[1]
          var_list[1] = *var_list_it;
          *var_list_it = var;
          // Add to new watch
          watch_list_manager_add_to_watch(&na->wlm, var_list_ref, var_list[1]);
    ...
    if (*var_list_it == variable_null) {
      if (ctx_trace_enabled(na->ctx, "na::propagate")) {
        ctx_trace_printf(na->ctx, "no watch found\n");
      }
      if (!na_plugin_has_assignment(na, *var_list)) {
        // We're unit
        constraint_unit_info_set(&na->unit_info, constraint_var, *var_list, CONSTRAINT_UNIT);
        // Process the constraint
        if (trail_is_consistent(trail)) {
          na_plugin_process_unit_constraint(na, prop, constraint_var);
        }
      } else {
        // Fully assigned
...
#ifndef NDEBUG
static
bool na_plugin_check_assignment(na_plugin_t* na) {
  if (!ctx_trace_enabled(na->ctx, "na::check_assignment")) {
    return true;
  }

  const mcsat_trail_t* trail = na->ctx->trail;
  const variable_db_t* var_db = na->ctx->var_db;
  const lp_data_t* lp_data = &na->lp_data;
  ...
  for (i = 0; i < trail->elements.size; ++ i) {
    ...
    if (x == na->last_decided_and_unprocessed) {
      continue;
    }
    const mcsat_value_t* value = trail_get_value(trail, x);
    if (value->type == VALUE_LIBPOLY && na_plugin_has_assignment(na, x)) {
...
void na_plugin_propagate(plugin_t* plugin, trail_token_t* prop) {
  ...
  if (ctx_trace_enabled(na->ctx, "na::propagate")) {
    ctx_trace_printf(na->ctx, "trail:\n");
    trail_print(trail, na->ctx->tracer->file);
  }
  ...
  while (trail_is_consistent(trail) && na->trail_i < trail_size(trail)) {
    ...
    if (constraint_unit_info_has(&na->unit_info, var)) {
      ...
      case CONSTRAINT_UNIT:
        ...
        na_plugin_process_unit_constraint(na, prop, var);
        break;
...
static
void na_plugin_decide(plugin_t* plugin, variable_t x, trail_token_t* decide_token, bool must) {
  na_plugin_t* na = (na_plugin_t*) plugin;

  assert(variable_db_is_real(na->ctx->var_db, x) || variable_db_is_int(na->ctx->var_db, x));

  // Get the feasibility set
  const lp_feasibility_set_t* feasible = feasible_set_db_get(na->feasible_set_db, x);

  if (ctx_trace_enabled(na->ctx, "na::decide")) {
    ctx_trace_printf(na->ctx, "decide on ");
    variable_db_print_variable(na->ctx->var_db, x, ctx_trace_out(na->ctx));
    ctx_trace_printf(na->ctx, "[%d] at level %d\n", x, na->ctx->trail->decision_level);
    if (feasible) {
      ctx_trace_printf(na->ctx, "feasible :");
      feasible_set_db_print_var(na->feasible_set_db, x, ctx_trace_out(na->ctx));
      ctx_trace_printf(na->ctx, "\n");
    } else {
  ...
  if (decide) {
    ...
    if (ctx_trace_enabled(na->ctx, "na::decide")) {
      ctx_trace_printf(na->ctx, "decided on ");
      variable_db_print_variable(na->ctx->var_db, x, ctx_trace_out(na->ctx));
      ctx_trace_printf(na->ctx, "[%d]: ", x);
      if (using_cached) {
        mcsat_value_print(x_cached_value, ctx_trace_out(na->ctx));
      } else {
        lp_value_print(&x_new_lpvalue, ctx_trace_out(na->ctx));
      }
      ctx_trace_printf(na->ctx, "\n");
    }
 ...
 */
static
void na_plugin_check_conflict(na_plugin_t* na, ivector_t* core) {
  uint32_t i;
  ...
  for (i = 0; i < core->size; ++ i) {
    ...
    na_plugin_get_constraint_variables(na, constraint_term, &core_vars);
  }
...
#ifndef NDEBUG
      const lp_value_t* x_value_lp_in_assignment = lp_assignment_get_value(na->lp_data.lp_assignment, x_lp);
...
static
void na_plugin_get_real_conflict(na_plugin_t* na, const int_mset_t* pos, const int_mset_t* neg,
    variable_t x, ivector_t* conflict) {
  size_t i;

  if (ctx_trace_enabled(na->ctx, "na::conflict")) {
    ctx_trace_printf(na->ctx, "na_plugin_get_conflict(): ");
  ...
  if (ctx_trace_enabled(na->ctx, "na::conflict")) {
    ...
    for (i = 0; i < core.size; ++ i) {
      ...
      if (trail_has_value(na->ctx->trail, core.data[i])) {
        mcsat_value_print(trail_get_value(na->ctx->trail, core.data[i]), ctx_trace_out(na->ctx));
      }
      ctx_trace_printf(na->ctx, "): ");
      ctx_trace_term(na->ctx, variable_db_get_term(na->ctx->var_db, core.data[i]));
    }
  }

  if (ctx_trace_enabled(na->ctx, "na::check_conflict")) {
    na_plugin_check_conflict(na, &core);
  }

  // Project
  na_plugin_explain_conflict(na, pos, neg, x, &core, &lemma_reasons, conflict);

  if (ctx_trace_enabled(na->ctx, "na::conflict")) {
    ctx_trace_printf(na->ctx, "na_plugin_get_conflict(): conflict:\n");
    for (i = 0; i < conflict->size; ++ i) {
      ctx_trace_printf(na->ctx, "[%zu]: ", i);
      ctx_trace_term(na->ctx, conflict->data[i]);
    }
  }
 ...
 */
static
bool na_plugin_speculate_constraint(na_plugin_t* na, int_mset_t* pos, int_mset_t* neg,
  ...
  variable_t constraint_var = variable_db_get_variable(na->ctx->var_db, constraint_atom);
  na_poly_constraint_add(na, constraint_var);

  // Check if the constraint is in Boolean conflict
  if (trail_has_value(na->ctx->trail, constraint_var)) {
    if (trail_get_boolean_value(na->ctx->trail, constraint_var) == negated) {
  ...
  lp_feasibility_set_t* constraint_feasible = na_plugin_get_feasible_set(na, constraint_var, x, negated);

  // Update the infeasible intervals
  bool feasible = feasible_set_db_update(na->feasible_set_db, x, constraint_feasible, &constraint_var, 1);
  ...
  if (!feasible) {
    ...
    na_plugin_get_real_conflict(na, pos, neg, x, &constraint_conflict);
...
static
void na_plugin_get_int_conflict(na_plugin_t* na, int_mset_t* pos, int_mset_t* neg,
    variable_t x, ivector_t* conflict) {
  ...
  term_t x_term = variable_db_get_term(na->ctx->var_db, x);
  ...
  for (;;) {
    ...
    const lp_feasibility_set_t* x_feasible = feasible_set_db_get(na->feasible_set_db, x);
    if (ctx_trace_enabled(na->ctx, "nia")) {
      ctx_trace_printf(na->ctx, "x = ");
      variable_db_print_variable(na->ctx->var_db, x, ctx_trace_out(na->ctx));
      ctx_trace_printf(na->ctx, "\nx_feasible = ");
      lp_feasibility_set_print(x_feasible, ctx_trace_out(na->ctx));
      ctx_trace_printf(na->ctx, "\n");
    }
    lp_feasibility_set_pick_first_value(x_feasible, &v);
    if (ctx_trace_enabled(na->ctx, "nia")) {
      ctx_trace_printf(na->ctx, "v = ");
      lp_value_print(&v, ctx_trace_out(na->ctx));
      ctx_trace_printf(na->ctx, "\n");
    }
    ...
    term_t v_floor_term = mk_arith_constant(na->ctx->tm, &v_floor_rat);
    ...
    // The constraint
    term_t x_leq_floor = mk_arith_leq(na->ctx->tm, x_term, v_floor_term);
    int_mset_add(&to_resolve, x_leq_floor);

    // Get the conflict
    feasible_set_db_push(na->feasible_set_db);
    bool feasible = na_plugin_speculate_constraint(na, pos, neg, x, x_leq_floor, conflict);
    assert(!feasible);
    ...
    term_t v_ceil_term = mk_arith_constant(na->ctx->tm, &v_ceil_rat);
    ...
    // The constraint
    term_t x_geq_ceil = mk_arith_geq(na->ctx->tm, x_term, v_ceil_term);
    int_mset_add(&to_resolve, x_geq_ceil);

    // Try it out
    feasible = na_plugin_speculate_constraint(na, pos, neg, x, x_geq_ceil, conflict);

    if (ctx_trace_enabled(na->ctx, "nia")) {
      ctx_trace_printf(na->ctx, "int_conflict: current conflict:\n");
      uint32_t i;
      for (i = 0; i < conflict->size; ++ i) {
        ctx_trace_printf(na->ctx, "  ");
        ctx_trace_term(na->ctx, conflict->data[i]);
  ...
  if (ctx_trace_enabled(na->ctx, "nia")) {
    ctx_trace_printf(na->ctx, "int_conflict: final conflict:\n");
    for (uint32_t j = 0; j < conflict->size; ++ j) {
      ctx_trace_printf(na->ctx, "  ");
      ctx_trace_term(na->ctx, conflict->data[j]);
    }
  }
...
static
void na_plugin_get_assumption_conflict(na_plugin_t* na, variable_t x, ivector_t* conflict) {
  size_t i;

  if (ctx_trace_enabled(na->ctx, "na::conflict")) {
    ctx_trace_printf(na->ctx, "na_plugin_get_assumption_conflict(): ");
    ctx_trace_term(na->ctx, variable_db_get_term(na->ctx->var_db, x));
  }

  // Get the value of x (responsible for conflict)
  const mcsat_value_t* x_value = trail_get_value(na->ctx->trail, x);

  // The assertions on x that are in conflict (as constraint variables)
  ivector_t core, lemma_reasons;
  init_ivector(&core, 0);
  init_ivector(&lemma_reasons, 0);
  feasible_set_db_get_conflict_reasons(na->feasible_set_db, x, x_value, &core, &lemma_reasons);

  if (ctx_trace_enabled(na->ctx, "na::conflict")) {
    ctx_trace_printf(na->ctx, "na_plugin_get_assumption_conflict(): core:\n");
    for (i = 0; i < core.size; ++ i) {
      ctx_trace_printf(na->ctx, "[%zu] (", i);
      if (trail_has_value(na->ctx->trail, core.data[i])) {
        mcsat_value_print(trail_get_value(na->ctx->trail, core.data[i]), ctx_trace_out(na->ctx));
  ...
  if (lemma_reasons.size > 0) {
    ...
    variable_t var = na->conflict_variable_assumption;
    lp_data_variable_order_push(&na->lp_data);
    term_t t = variable_db_get_term(na->ctx->var_db, var);
    lp_data_add_to_model_and_context(&na->lp_data, lp_data_get_lp_variable_from_term(&na->lp_data, t), &na->conflict_variable_value);
    for (i = 0; i < lemma_reasons.size; ++ i) {
      ...
      bool ok = poly_constraint_evaluate(constraint, &na->lp_data, &constraint_value);
      (void) ok;
      assert(ok);
    ...
    assert(trail_has_value(na->ctx->trail, constraint_var));

    bool constraint_value = trail_get_boolean_value(na->ctx->trail, constraint_var);
    ...
    const variable_t* var_i = watch_list_manager_get_list(&na->wlm, var_list_ref);
    bool constraint_evaluates = true;
    while (constraint_evaluates && *var_i != variable_null) {
      if (!trail_has_value(na->ctx->trail, *var_i)) {
    ...
    if (constraint_evaluates) {
      ...
      term_t p_reason = lp_polynomial_to_yices_term_na(na, p_reason_lp);

      // Get the sign of the polynomial
      assert(trail_has_value(na->ctx->trail, x));
      assert(lp_assignment_get_value(na->lp_data.lp_assignment, x_lp)->type == LP_VALUE_NONE);
      lp_assignment_set_value(na->lp_data.lp_assignment, x_lp, &na->conflict_variable_value);
      int sgn = lp_polynomial_sgn(p_reason_lp, na->lp_data.lp_assignment);
      if (ctx_trace_enabled(na->ctx, "na::conflict")) {
        ctx_trace_printf(na->ctx, "p_reason = ");
        ctx_trace_term(na->ctx, p_reason);
        lp_assignment_print(na->lp_data.lp_assignment, ctx_trace_out(na->ctx));
      }
      lp_assignment_set_value(na->lp_data.lp_assignment, x_lp, 0);

      // Construct the explanation
      term_t reason = NULL_TERM;
      if (sgn == 0) {
        reason = mk_arith_term_eq0(na->ctx->tm, p_reason);
      } else if (sgn > 0) {
        reason = mk_arith_term_gt0(na->ctx->tm, p_reason);
      } else {
        reason = mk_arith_term_lt0(na->ctx->tm, p_reason);
      }
  ...
  if (ctx_trace_enabled(na->ctx, "na::conflict")) {
    ctx_trace_printf(na->ctx, "na_plugin_get_assumption_conflict(): conflict:\n");
    for (i = 0; i < conflict->size; ++ i) {
      ctx_trace_printf(na->ctx, "[%zu]: ", i);
      ctx_trace_term(na->ctx, conflict->data[i]);
    }
...
static
void na_plugin_get_conflict(plugin_t* plugin, ivector_t* conflict) {
  na_plugin_t* na = (na_plugin_t*) plugin;

  if (ctx_trace_enabled(na->ctx, "na::conflict")) {
    ctx_trace_printf(na->ctx, "na_plugin_get_conflict(): START\n");
  }
  ...
  if (na->conflict_variable != variable_null) {
    na_plugin_get_real_conflict(na, &pos, &neg, na->conflict_variable, conflict);
  ...
  if (ctx_trace_enabled(na->ctx, "na::conflict")) {
    uint32_t i;
    ctx_trace_printf(na->ctx, "na_plugin_get_conflict(): END\n");
    for (i = 0; i < conflict->size; ++ i) {
      ctx_trace_term(na->ctx, conflict->data[i]);
    }
  }
...
bool na_plugin_explain_evaluation(plugin_t* plugin, term_t t, int_mset_t* vars, mcsat_value_t* value) {
  ...
  na_plugin_get_constraint_variables(na, t, vars);

  // Check if the variables are assigned
  ivector_t* var_list = int_mset_get_list(vars);
  for (size_t i = 0; i < var_list->size; ++ i) {
    if (!trail_has_value(na->ctx->trail, var_list->data[i])) {
      result = false;
    }
  }
...
static
void na_plugin_push(plugin_t* plugin) {
  na_plugin_t* na = (na_plugin_t*) plugin;
  scope_holder_push(&na->scope,
      &na->trail_i,
      &na->processed_variables_size,
      NULL);
...
void na_plugin_pop(plugin_t* plugin) {
  ...
  if (ctx_trace_enabled(na->ctx, "na::conflict")) {
    trail_print(trail, ctx_trace_out(na->ctx));
  }
  ...
  while (na->processed_variables.size > na->processed_variables_size) {
    // The variable to undo
    variable_t x = ivector_last(&na->processed_variables);
    ivector_pop(&na->processed_variables);
    assert(variable_db_is_real(na->ctx->var_db, x) || variable_db_is_int(na->ctx->var_db, x));
    // Go through the watch and mark the constraints
    remove_iterator_t it;
    remove_iterator_construct(&it, &na->wlm, x);
    while (!remove_iterator_done(&it)) {
      variable_t constraint_var = remove_iterator_get_constraint(&it);
      constraint_unit_info_demote(&na->unit_info, constraint_var, x);
      remove_iterator_next_and_keep(&it);
    }
    remove_iterator_destruct(&it);
  }
...
static
void na_plugin_gc_mark(plugin_t* plugin, gc_info_t* gc_vars) {
  na_plugin_t* na = (na_plugin_t*) plugin;
  ...
  feasible_set_db_gc_mark(na->feasible_set_db, gc_vars);
  // We also need to mark all the real variables that are in use
  watch_list_manager_gc_mark(&na->wlm, gc_vars);
}

static
void na_plugin_gc_sweep(plugin_t* plugin, const gc_info_t* gc_vars) {
  na_plugin_t* na = (na_plugin_t*) plugin;
  ...
  poly_constraint_db_gc_sweep(na->constraint_db, na->ctx, gc_vars);

  // The lp_data mappings
  lp_data_gc_sweep(&na->lp_data, gc_vars);

  // Evaluation cache
  gc_info_sweep_int_hmap_keys(gc_vars, &na->evaluation_value_cache);
  gc_info_sweep_int_hmap_keys(gc_vars, &na->evaluation_timestamp_cache);

  // Feasible set cache
  gc_info_sweep_int_hmap_keys(gc_vars, &na->feasible_set_cache_top_var[0]);
  gc_info_sweep_int_hmap_values(gc_vars, &na->feasible_set_cache_top_var[0]);
  gc_info_sweep_int_hmap_keys(gc_vars, &na->feasible_set_cache_top_var[1]);
  gc_info_sweep_int_hmap_values(gc_vars, &na->feasible_set_cache_top_var[1]);
  gc_info_sweep_int_hmap_keys(gc_vars, &na->feasible_set_cache_timestamp[0]);
  gc_info_sweep_int_hmap_keys(gc_vars, &na->feasible_set_cache_timestamp[1]);
  gc_info_sweep_ptr_hmap_keys(gc_vars, &na->feasible_set_cache[0], (ptr_hmap_ptr_delete) &lp_feasibility_set_delete);
  gc_info_sweep_ptr_hmap_keys(gc_vars, &na->feasible_set_cache[1], (ptr_hmap_ptr_delete) &lp_feasibility_set_delete);

  // Unit information (constraint_unit_info, constraint_unit_var)
  constraint_unit_info_gc_sweep(&na->unit_info, gc_vars);

  // Watch list manager
  watch_list_manager_gc_sweep_lists(&na->wlm, gc_vars);
}
...
void na_plugin_new_lemma_notify(plugin_t* plugin, ivector_t* lemma, trail_token_t* prop) {
  ...
  for (i = 0; unit && i < lemma->size; ++ i) {
    ...
    if (ctx_trace_enabled(na->ctx, "na::lemma")) {
      ctx_trace_printf(na->ctx, "lemma[%u] = ", i); ctx_trace_term(na->ctx, lemma->data[i]); ctx_trace_printf(na->ctx, "\n");
    }

    term_t literal = lemma->data[i];
    term_t atom = unsigned_term(literal);
    variable_t atom_var = variable_db_get_variable_if_exists(na->ctx->var_db, atom);
    assert(atom_var != variable_null);
    if (constraint_unit_info_get(&na->unit_info, atom_var) != CONSTRAINT_UNIT) {
      // Not unit
      unit = false;
    } else {
      // Unit, check if same variable
      variable_t atom_unit_var = constraint_unit_info_get_unit_var(&na->unit_info, atom_var);
  ...
  if (unit && na->ctx->trail->decision_level == 0) {
    ...
    for (i = 0; i < lemma->size; ++ i) {
      ...
      if (ctx_trace_enabled(na->ctx, "na::lemma")) {
        ctx_trace_printf(na->ctx, "na: lemma_feasible = ");
        lp_feasibility_set_print(lemma_feasible, ctx_trace_out(na->ctx));
        ctx_trace_printf(na->ctx, "\n");
      }

      term_t literal_term = lemma->data[i];
      term_t constraint_term = unsigned_term(literal_term);
      variable_t constraint_var = variable_db_get_variable_if_exists(na->ctx->var_db, constraint_term);
      ...
      lp_feasibility_set_t* constraint_feasible = na_plugin_get_feasible_set(na, constraint_var, unit_var, negated);
    ...
    if (!lp_feasibility_set_is_full(lemma_feasible)) {
      ...
      if (ctx_trace_enabled(na->ctx, "na::lemma")) {
        const lp_feasibility_set_t* current_feasible = feasible_set_db_get(na->feasible_set_db, unit_var);
        if (current_feasible) {
          ctx_trace_printf(na->ctx, "na: current feasible = ");
          lp_feasibility_set_print(current_feasible, ctx_trace_out(na->ctx));
          ctx_trace_printf(na->ctx, "\n");
        }
      }
      ...
      // Update
      bool feasible = feasible_set_db_update(na->feasible_set_db, unit_var, lemma_feasible, lemma_reasons.data, lemma_reasons.size);
      if (ctx_trace_enabled(na->ctx, "na::lemma")) {
        ctx_trace_printf(na->ctx, "na: new feasible = ");
        const lp_feasibility_set_t* current_feasible = feasible_set_db_get(na->feasible_set_db, unit_var);
        lp_feasibility_set_print(current_feasible, ctx_trace_out(na->ctx));
      ...
      if (!feasible) {
        ...
        na_plugin_report_conflict(na, prop, unit_var);
      } else if (variable_db_is_int(na->ctx->var_db, unit_var) && !lp_feasibility_set_contains_int(feasible_set_db_get(na->feasible_set_db, unit_var))) {
        // If not integer value for integer variable, report int conflict
        na_plugin_report_int_conflict(na, prop, unit_var);
      }
...
static
void na_plugin_set_exception_handler(plugin_t* plugin, jmp_buf* handler) {
  na_plugin_t* na = (na_plugin_t*) plugin;
  na->exception = handler;
}

static
void na_plugin_decide_assignment(plugin_t* plugin, variable_t x, const mcsat_value_t* value, trail_token_t* decide) {
  na_plugin_t* na = (na_plugin_t*) plugin;
  // If we get a rational, convert to lp_value_t
  mcsat_value_t tmp;
  const mcsat_value_t* lp_value = ensure_lp_value(value, &tmp);
  // Get the feasibility set
  lp_feasibility_set_t* feasible = feasible_set_db_get(na->feasible_set_db, x);
  ...
  if (feasible != NULL && !lp_feasibility_set_contains(feasible, &lp_value->lp_value)) {
    // Ouch, conflict
    if (ctx_trace_enabled(na->ctx, "mcsat::decide")) {
      ctx_trace_printf(na->ctx, "decide conflict!\nfeasible = ");
      lp_feasibility_set_print(feasible, ctx_trace_out(na->ctx));
    }
    na_plugin_report_assumption_conflict(na, decide, x, lp_value);
  }
  // Remove temps
  if (lp_value != value) {
...
static
void na_plugin_learn(plugin_t* plugin, trail_token_t* prop) {
  ...
  if (ctx_trace_enabled(na->ctx, "mcsat::na::learn")) {
    ctx_trace_printf(na->ctx, "na_plugin_learn(): trail = ");
    trail_print(trail, ctx_trace_out(na->ctx));
  }

  // Get constraints at
  // - constraint_db->constraints
  const ivector_t* all_constraint_vars = poly_constraint_db_get_constraints(na->constraint_db);
  for (uint32_t i = 0; i < all_constraint_vars->size; ++ i)  {
    ...
    if (has_value) {
      if (trail_get_source_id(trail, constraint_var) == na->ctx->plugin_id) {
    ...
    constraint_value = na_poly_constraint_db_approximate(na, constraint_var);
    if (ctx_trace_enabled(na->ctx, "mcsat::na::learn")) {
      ctx_trace_printf(na->ctx, "na_plugin_learn(): value = ");
      if (constraint_value != NULL) {
        mcsat_value_print(constraint_value, ctx_trace_out(na->ctx));
      } else {
        ctx_trace_printf(na->ctx, "no value");
      }
    }
    if (constraint_value != NULL) {
      if (has_value) {
        ...
        if (existing_value != constraint_value->b) {
          ...
          na_plugin_report_conflict(na, prop, variable_null);
          break;
        }
      } else {
        if (!na->ctx->options->model_interpolation) {
          prop->add(prop, constraint_var, constraint_value);
        } else {
          if (ctx_trace_enabled(na->ctx, "na::learn")) {
            ctx_trace_printf(na->ctx, "na: hinting variable = %d\n", constraint_var);
          }
          na->ctx->hint_next_decision(na->ctx, constraint_var);
          // update the trail value cache
          na->ctx->hint_value(na->ctx, constraint_var, constraint_value);
        }
      }
    }
  }
...
bool na_plugin_simplify_conflict_literal(plugin_t* plugin, term_t lit, ivector_t* output) {
  ...
  term_kind_t lit_kind = term_kind(na->ctx->terms, lit_pos);
  if (lit_kind != ARITH_ROOT_ATOM) {
    return false;
  }

  if (ctx_trace_enabled(na->ctx, "na::simplify_conflict")) {
    ctx_trace_printf(na->ctx, "Simplifying conflict literal\n");
    trail_print(na->ctx->trail, ctx_trace_out(na->ctx));
    ctx_trace_term(na->ctx, lit_pos);
  }

  // Get the polynomial of the root atom
  root_atom_t* root_atom = arith_root_atom_desc(na->ctx->terms, lit_pos);
  term_t lit_p = root_atom->p;

  // Cell will be described as a conjunction of true atoms
  na_plugin_describe_cell(na, lit_p, output);

  if (ctx_trace_enabled(na->ctx, "na::simplify_conflict")) {
    ctx_trace_printf(na->ctx, "Simplifying done\n");
    for (; start < output->size; ++ start) {
      ctx_trace_printf(na->ctx, "[%d] = ", start);
      ctx_trace_term(na->ctx, output->data[start]);
    }
  }
...
plugin_t* na_plugin_allocator(void) {
  na_plugin_t* plugin = safe_malloc(sizeof(na_plugin_t));
  plugin_construct((plugin_t*) plugin);
  plugin->plugin_interface.construct           = na_plugin_construct;
  ...
  plugin->plugin_interface.learn               = na_plugin_learn;
  plugin->plugin_interface.get_conflict        = na_plugin_get_conflict;
  plugin->plugin_interface.explain_propagation = na_plugin_explain_propagation;
  plugin->plugin_interface.explain_evaluation  = na_plugin_explain_evaluation;
  plugin->plugin_interface.simplify_conflict_literal = na_plugin_simplify_conflict_literal;
  plugin->plugin_interface.push                = na_plugin_push;
  plugin->plugin_interface.pop                 = na_plugin_pop;
  plugin->plugin_interface.gc_mark             = na_plugin_gc_mark;
  plugin->plugin_interface.gc_sweep            = na_plugin_gc_sweep;
